...
- 챕터/주제 제목 및 질문 범위는 다음 우선순위로 결정한다:
  1. 사용자가 직접 언급한 주제 및 질문 범위 (예: "1장 Q1~Q5", "HTML 기본 구조 (Q1~Q5)")
...
PR 제목 규칙: `[이름] 0N주차: <선정 주제 또는 챕터> (<질문 범위>)`  
(예: `[이만재] 01주차: 1장 HTML과 웹 기본 구조 (Q1~Q5)`)

```bash
gh pr create --base main --title "[이름] 0N주차: 챕터 주제 (Q1~Q5)" --body "$(cat <<'EOF'
...
## ✅ 체크리스트
- [x] 브랜치명이 `0N주차/<이름>` 형식인가요?
- [x] 파일/폴더 경로가 `0N주차/<이름>...` 에 맞게 배치되었나요?
- [x] 불필요한 파일이 커밋에 포함되지 않았나요?
EOF
)"
```

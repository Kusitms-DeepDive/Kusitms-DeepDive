...
description: >-
  ...
  only say "올려줘", "PR 만들어줘", or mention a file under 0N주차/*.md or N주차/*.md.
---
...
- `0N주차/<이름>.md` (또는 `N주차/<이름>.md`) 정리본을 커밋·푸시·PR 할 때
...
- 경로 패턴: `0N주차/<이름>.md` (예: `01주차/이만재.md` → 주차=`01주차`, 이름=`이만재`)
- 만약 사용자가 `1주차/이만재.md`처럼 한 자리 숫자로 입력하더라도 레포지토리의 `01주차/<이름>.md` 포맷을 우선 확인하고 통일한다.
- 챕터/주제 제목은 다음 우선순위로 결정한다:
  1. 사용자가 직접 언급한 주제
  2. `0N주차/<이름>.md` 파일 상단의 `# 제목` 헤딩
